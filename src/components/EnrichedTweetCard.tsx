...
interface EnrichedPostCardProps {
  post: EnrichedPost;
}

function EnrichedPostCard({ post }: EnrichedPostCardProps) {
    ...
    <article className="px-4 py-3 border-b border-[#2F3336] hover:bg-[#080808] transition-colors cursor-pointer">
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Engagement Actions - X Style - Clickable */}
          <div className="flex items-center justify-between mt-3 max-w-[425px]">
            {/* Reply */}
            <button
              onClick={(e) => {
                e.stopPropagation();
                window.open(`https://twitter.com/intent/tweet?in_reply_to=${post.id}`, '_blank', 'width=550,height=420');
              }}
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                window.open(`https://twitter.com/intent/retweet?tweet_id=${post.id}`, '_blank', 'width=550,height=420');
              }}
              className="flex items-center group"
              title="Repost"
            >
            ...
            <button
              onClick={(e) => {
                e.stopPropagation();
                window.open(`https://twitter.com/intent/like?tweet_id=${post.id}`, '_blank', 'width=550,height=420');
              }}
              className="flex items-center group"
              title="Like"
            >
            ...
            {/* Quote / Bookmark / Share */}
            <div className="flex items-center">
              {/* Quote Post */}
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  const postUrl = `https://twitter.com/${post.author.username}/status/${post.id}`;
                  window.open(`https://twitter.com/intent/tweet?url=${encodeURIComponent(postUrl)}`, '_blank', 'width=550,height=420');
                }}
...
export default memo(EnrichedPostCard);

// Legacy export for backward compatibility
export { EnrichedPostCard as EnrichedTweetCard };
